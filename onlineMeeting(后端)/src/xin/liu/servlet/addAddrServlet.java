...
public class addAddrServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String addAddrname = request.getParameter("addAddrname");
		String addAddrStatus = request.getParameter("addAddrStatus");
		...
		try{
	        ...
	        // SQL语句
			//判断是否存在此用户
	        String sqlCount = "select * from address order by addressID desc";
	        ...
	        int addressID = 0;
	        if(resultCount.next()){
	        	addressID = resultCount.getInt(1);
	        	addressID++;
            }
	        String sqlInsert = "insert into address values ('"+addressID+"', '"+addAddrname+"', '"+addAddrStatus+"', "
	        		+ "'"+openTime+"', '"+closeTime+"', '"+createUser+"')";
