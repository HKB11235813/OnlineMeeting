...
/**
 * Servlet implementation class venueManageServlet
 */
@WebServlet("/venueManageServlet")
public class venueManageServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public venueManageServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			String sql = "SELECT * FROM address where meetingAddress like '%"+meetingAddress+"%' "
					+ "and addressStatus like '"+addressStatus+"%' "
	        		+ "limit "+((currentPage-1)*5)+","+(currentPage*5);
	        String sqlStr = "select count(*) from address where meetingAddress like '%"+meetingAddress+"%' "
					+ "and addressStatus like '"+addressStatus+"%' ";
	        ...
	        while(rs.next()){
	        	...
	        	address.setAddressStatus(rs.getString("addressStatus"));
	        	address.setOpenTime(sdf.format(rs.getDate("openTime")));
	        	address.setCloseTime(sdf.format(rs.getDate("closeTime")));
	        	address.setCreateUser(rs.getString("createUser"));
	        	address.setTotalPage(totalPage);
	        	address.setTotalRecord(totalRecord);
